...
if point_distance(x,y,target.x,target.y) < 16
{
	if furnacing_ore = noone && (global.inhand_item_type = spr_only_items || global.inhand_item_type = spr_inv_item)
	{
		for(var i = 0; i < 36; i += 2)
		{
			if i != 14 && i != 16 && i != 22
			{
				if global.inhand_item = i && (inventory_number_find_by_name(asset_get_index("item"+string(global.inhand_item+1)),0) >= 2) && (inventory_number_find_by_name(item17,0) >= 1)
				{
				furnacing_ore = asset_get_index("item"+string(global.inhand_item+2))
				furnacing_time = 60
				inventory_item_sub(asset_get_index("item"+string(global.inhand_item+1)),2,0)
				inventory_item_sub(item17,1,0)
