...
if inside_of_view = 1
{
	...
	else
	{
		if instance_number(player_co_op) > 0
		{
			...
			var b_txscale = txscale
			auto_picking = 1.2
			set_global_hp("Electric battert", txscale*2, 1)
			txscale -= global.pickaxe_power/30000
			tyscale -= global.pickaxe_power/30000
	...
	if noclip_block != noone
	{
		...
		{
		depth = -((y)+0.5*34)
		noclip_block.image_xscale = image_xscale*0.6
		noclip_block.image_yscale = image_yscale*0.3
		noclip_block.x = x
		noclip_block.y = y
		}
		else
