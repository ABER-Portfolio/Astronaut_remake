...
if found_human = 1 && bullet_shoot_count > 0
{
	if (image_index >= 3 && image_index <= 5) || (image_index >= 9 && image_index <= 11)
	{
	draw_sprite_ext(spr_pickaxe,random_bow,floor(x),floor(y)+7,-0.3,gun_yscale,t_weapon_angle,image_blend,image_alpha)
	}
	else
	{
	draw_sprite_ext(spr_pickaxe,random_bow,floor(x),floor(y)+9,-0.3,gun_yscale,t_weapon_angle,image_blend,image_alpha)
	}
}
