...
if (global.drawing_cancle_ui = 0 || global.inv_open > 0) && global.inv_open = 0 && global.draw_minimap != 1
{
if global.chat_activity = false && global.playing_scene = 0 && obj_camera.scene_cancle_cooltime > 2
{
...
draw_sprite_ext(sprite_index,0,xx+(box_size*x_scale*box_num),yy-(y_scale*box_size*0.8),xx_scale*x_scale,yy_scale*y_scale,0,c_white,0.65)//0.85
	...
	if item_num != noone
	{
		...
		{
			if item_real_spr = spr_accessories
			{
			draw_sprite_ext(item_real_spr,item_num,xx+(box_size*x_scale*box_num),yy-(y_scale*box_size*0.8),(xx_scale*x_scale*1.75),(yy_scale*y_scale*1.75),0,c_white,1)
			}
			else
			{
				if item_real_spr = spr_clothes_center_origin
				{
				draw_sprite_ext(item_real_spr,item_num,xx+(box_size*x_scale*box_num),yy-(y_scale*box_size*0.8),(xx_scale*x_scale*1.1),(yy_scale*y_scale*1.1),0,c_white,1)
				}
				else
				{
					if item_real_spr = spr_potion_center_origin || item_real_spr = spr_inv_item || item_real_spr = spr_only_items
					{
					draw_sprite_ext(item_real_spr,item_num,xx+(box_size*x_scale*box_num),yy-(y_scale*box_size*0.8),(xx_scale*x_scale*1.3),(yy_scale*y_scale*1.3),0,c_white,1)
					}
					else
					{
						if item_real_spr != none_sprite
						{
