...
{
	if instance_number(player_co_op) > 0 && cool_time > 0 && player_co_op.spin = 0
	{
...
if instance_number(player_co_op) > 0 && global.go_to_other_room = 0
{
	if instance_number(BG_blind) = 0
	{
		if co_op_code.is_server = false
		{
			...
			else
			{
				...
				{
				light.x = player_co_op.x
				light.y = player_co_op.y
				}
			light.lightxs = 96
			light.lightys = 96
			...
			{
			light_p.x = player_co_op.x
			light_p.y = player_co_op.y
			light_p.lightxs = 128
			light_p.lightys = 128
...
if(co_op_code.is_server == false)
{
global.attack_damage_co_op = global.attack_damage
	...
	{
	y = (player_co_op.y+5)
		if cool_time = 1 && player_co_op.spin = 0
		{
			if scale = 1
			{
			player_co_op.image_xscale = abs(player_co_op.image_xscale)*(-1)
			}
			else
			{
		...
		image_xscale = (global.tool_size*(-abs(player_co_op.image_xscale)/player_co_op.image_xscale))
	}
	...
	image_xscale = global.tool_size
		...
		image_angle = angle_+sign(player_co_op.image_xscale)*45
	...
	buffer_write(chat_buffer, buffer_u8, DATA.P2_INHAND);
	...
	{
	buffer_seek(chat_buffer, buffer_seek_start, 0);
	buffer_write(chat_buffer, buffer_u8, DATA.CO_OP_SWING);
	buffer_write(chat_buffer, buffer_string, global.swing_co_op);
