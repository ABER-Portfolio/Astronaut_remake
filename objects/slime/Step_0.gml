...
if inside_item != none_sprite
{
inside_item_angle_time ++
	if inside_item_angle_time > 110
	{
	inside_item_angle_turn ++
	inside_item_angle_time = 0
	}
	
	if inside_item_angle_turn > 1
	{
	inside_item_angle_turn = 0
	}
	
	
	if inside_item_angle_turn = 0
	{
	inside_item_angle += 0.1
	}
	else
	{
	inside_item_angle -= 0.1
...
if co_op_code.is_server = true
{
	...
	image_speed = 0.3+stop_moving/180

	depth = -(y+40*image_yscale)



	if dilay = 0
	{
	speed += (t_speed - speed)*0.05
	stop_moving++

		if point_distance(speed,0,0,0) <= 0.1 && stop_moving > 180
		{
		...
		image_index = 0
		move_towards_point(target_x,target_y+3,2.2)
		stop_moving = 0
		}
	...
	if instance_exists(target_p) && point_distance(speed,0,0,0) > 0.1
	{
		if x > target_p.x
		{
