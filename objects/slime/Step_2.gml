...
audio_sound_gain(die,7*global.master_volume*global.sfx_volume*global.scene_volume,1)
	...
	if co_op_code.is_server = true
	{
		for(var i = 0; i < choose(3,3,3,3,4,4); i++)
		{
		var tiny_sl = instance_create(x+choose(-13,13)*0.5,y+choose(-13,13)*0.5,tiny_slime)
		tiny_sl.received = 0
		tiny_sl.sprite_index = sprite_index
		tiny_sl.vspeed = irandom_range(-13,13)*0.1
		tiny_sl.hspeed = irandom_range(-13,13)*0.1
		}

	if inside_item = pl_move_projectile_hazmat
	{
	var dead_ef = instance_create(x,y,dead_head)
	dead_ef.image_index = 18

	var random_drop = percentage_k(4)
		if random_drop = 1
		{
		var random_drop_ = percentage_k(2)
			if random_drop_ = 0
			{
			...
			cre_item.image_index = irandom_range(43,49)
			cre_item.floor_y = y+8
...
cre_item.image_xscale = (cre_item.hspeed/abs(cre_item.hspeed))*0.18
cre_item.image_yscale = 0.18
cre_item.image_index = inside_item_index
cre_item.floor_y = y+8
cre_item.depth = depth
cre_item.received_item = 0
cre_item.a = irandom_range(9,15)
