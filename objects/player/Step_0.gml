...
if co_op_code.is_server = true
{
	...
	if global.co_op_mode = 1 && global.knocked_out_co_op > 0
	{
		if point_distance(x,y,player_co_op.x,player_co_op.y) < 16
		{
	...
	if global.wind_dir > 0
	{
		if image_xscale > 0
		{
		global.movement_speed += (-global.wind_dir*0.006 - global.movement_speed)*0.002
		}
		if image_xscale < 0
		{
		global.movement_speed += ((global.wind_dir)*0.006 - global.movement_speed)*0.002
	...
	if instance_number(BG_blind) != 0 || global.knocked_out_origin != 0 || (cannot_move != 0 && spin = 0) || global.do_not_move != 0 || global.do_not_move2 != 0
	{
	...
	if global.knocked_out_origin = 0 && global.do_not_move = 0 && global.chat_activity = false && spin = 0
	{
	...
	if _positive = 1
	{
	var _shape_speed = abs(global.hmovement_speed/2)
	if _shape_speed > 0
	{
	_shape_speed = 0.95
	}
	...
	x += global.movement_speed*_shape_speed
	}
	...
	if _positive = 1
	{
	var _shape_speed = abs(global.movement_speed/2)
	if _shape_speed > 0
	{
	_shape_speed = 0.95
	}
	...
	y += global.hmovement_speed*0.88*_shape_speed
	}
